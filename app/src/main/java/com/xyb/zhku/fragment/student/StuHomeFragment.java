...
public class StuHomeFragment extends BaseFragment {
    ...
    protected void init(View view) {
        ...
        viewPager.setOffscreenPageLimit(2); //默认只能缓存1个页面，所以这里设置缓存3个，当滑到第4个页面的时候，第1个页码就不会被销毁了
...
//        }
        ...
        slider.setPresetTransformer(SliderLayout.Transformer.Default); //默认指定的动画类型
        slider.setPresetIndicator(SliderLayout.PresetIndicators.Center_Bottom);    //指定圆点指示器的位置
        slider.setCustomAnimation(new DescriptionAnimation());  //定义一个描述动画
        slider.setDuration(4000);   //动画时长
    }

    class MyViewPageAdapter extends FragmentPagerAdapter {
        String[] stuTitle = new String[]{"教学", "学生工作", "其它"};
        ...
        public Fragment getItem(int position) {
            return StuHomeFragmentFactory.createFragment(position);
        }

        public int getCount() {
            return stuTitle.length;
        }

        @Override
        public CharSequence getPageTitle(int position) {
            return stuTitle[position];
        }
    }
